...
public class UsuarioDaoImpl implements UsuarioDao {
    ...
    public void getRptaIdentificacion(Mensaje msg, Usuario usuario, boolean isEncripted) throws Exception {
        ...
        sql.append("SELECT \n"
                ...
                + "CURRENT_TIMESTAMP FULL_FECHA_ACTUAL, coalesce(NU_INTENTO,0) + 1 NRO_INTENTO,IN_AD as inAD\n"
    ...
    public String insAccesoLog(DatosUsuarioLog data){
        ...
        try {
            ...
            System.out.println("COD_USER==>" + data.getCodUser());
            System.out.println("DE_IPPC==>" + data.getDeIppc());
            System.out.println("DE_NAMEPC==>" + data.getDeNamepc());
            System.out.println("DE_USERPC==>" + data.getDeUserpc());
            System.out.println("IN_SUCCESS==>" + data.getSuccess());
            e.printStackTrace();
        }
        return vReturn;             
    }
 ...
 @Override
    public String cambiarEstadoUsuario(String coEmpleado,String estado,String coUsuario){
        ...
        try {
            this.jdbcTemplate.update(sqlUpd.toString(), new Object[]{estado,coUsuario, coEmpleado});
            vReturn = "OK";
        }catch (Exception e) {
            vReturn = "NO_OK";
            //e.printStackTrace();
    ...
    public String delUserAplicaOpc(String coUse) {
        String vReturn = "NO_OK";
        StringBuilder sqlUpd = new StringBuilder();
        sqlUpd.append("DELETE FROM IDOSGD.seg_user_aplica_opc   \n" +
                        "WHERE  cod_user = ? \n" +
                        "AND  cod_aplica = '9'");
    ...
    public String delUserAplica(String coUse) {
        String vReturn = "NO_OK";
        StringBuilder sqlUpd = new StringBuilder();
        sqlUpd.append("DELETE FROM IDOSGD.seg_user_aplica   \n" +
                        "WHERE cod_aplica = '9' \n" +
                        "AND cod_user = ?");
