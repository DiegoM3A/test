...
//                + "NU_DES=?");
    ...
    public String updDocumentoObj(final DocumentoObjBean docObjBean) {
        ...
        try {
            int cant = this.jdbcTemplate.queryForObject("select count(nu_ann) from tdtv_archivo_doc where nu_ann = ? and nu_emi = ?", new Object[]{docObjBean.getNuAnn(), docObjBean.getNuEmi()}, Integer.class);
            if (cant > 0) {
                inInsert = false;
            } else {
                inInsert = true;
